          ...
          {active === item && children && (
            <div className="absolute top-[calc(100%_+_1.2rem)] left-1/2 transform -translate-x-1/2 pt-4">
              <motion.div
                transition={transition}
                layoutId="active"
                className="backdrop-blur-md bg-white/10 border border-white/20 rounded-2xl overflow-hidden shadow-xl"
              >
...
export const Menu = ({
  ...
  return (
    <nav
      ...
      className="relative rounded-full backdrop-blur-md bg-white/10 border border-white/20 shadow-lg flex justify-center space-x-8 px-8 py-4"
    >
      {children}
    </nav>
  );
